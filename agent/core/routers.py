...
@router.post("/peers", response_model=AgentAddPeerResponse)
async def add_peer(request: AgentAddPeerRequest):
...
async def get_wireguard_status():
    ...
    try:
        ...
        peers = [WireGuardPeer(**p) for p in peers_data]
        return WireGuardStatusResponse(interface=wg_manager.interface, peers=peers)
    except Exception as e:
